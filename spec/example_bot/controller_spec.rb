RSpec.describe ExampleBot::Controller, telegram_bot: :poller do

  let(:bot) { Telegram.bots[:example_bot] }
  let(:current_chat) { Fabricate(:example_bot_chat) }
    ...
    describe 'operation' do
      context 'when is successful' do
        describe 'auto answer by received' do
          let!(:auto_answer) { Fabricate(:example_bot_auto_answer, chat_id: current_chat.id) }
