    ...
    class Controller < Telegram::Bot::UpdatesController
      module Callbacks
        ...
        def sync_chat
          ...
          logger.info("> Synced chat: #{@current_chat.name}")
        end
        ...
        def sync_user
          ...
          logger.info("> Synced user: #{@current_user.name}")
        end
        ...
        def sync_chat_user
          ...
          logger.info("> Synced chat user: #{@current_chat_user.id}")
        end
        ...
        def on_user_left_chat
          ...
          logger.info("> [On user left chat]: Marked chat user #{chat_user.id} as deleted")
        end
        ...
        def sync_message
          ...
          logger.info("> Synced message: '#{@current_message.text}'")
        end
